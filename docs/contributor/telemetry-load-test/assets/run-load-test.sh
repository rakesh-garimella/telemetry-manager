...
function check_deployment_ready() {
    ...
    if [ "$CURRENT" == "$DESIRED" ]; then
        # echo "Deployment $TELEMETRY_MANAGER_DEPLOYMENT_NAME is ready."
        echo "ready"
    else
        # echo "Deployment $TELEMETRY_MANAGER_DEPLOYMENT_NAME is not ready. $CURRENT out of $DESIRED replicas are ready."
        echo "not ready"
    fi
}
